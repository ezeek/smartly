...
//alert(iconlookup['zmdi-lamp']);
    ...
    Iconpicker.prototype.filterIcons = function () {
        ...
        if(op.iconsetVersion != 'lastest' && typeof Iconpicker.ICONSET[op.iconset].allVersions != 'undefined'){
            ...
            icons = Object.keys(Iconpicker.ICONSET[op.iconset].icons);
...
console.log("else");
    ...
    Iconpicker.prototype.select = function (icon) {
        ...
        // code/name detection must not be checking startsWith against null
        if (icon === null || icon === undefined) {
          icon = '';
        }

        if (icon.startsWith(op.iconClassFix)) {
            op.selected = $.inArray(icon.replace(op.iconClassFix, ''), op.icons);
        } else {
            op.selected = $.inArray(icon, Object.values(Iconpicker.ICONSET[op.iconset].icons));
            icon = op.iconClassFix + Object.keys(Iconpicker.ICONSET[op.iconset].icons)[op.selected];
        }
        if (op.selected === -1) {
            op.selected = 0;
        ...
        if (icon !== '' && op.selected >= 0) {
            op.icon = icon;
            if(op.inline === false){
                el.find('input').val(icon + 'opinline');
...
*/
            ...
            if(icon === op.iconClassFix){
                ...
                el.find('input:hidden').val(''); //ELI icon
                el.find('input:text').val('');
            }
            else {

console.log(el.find('input'));
                el.trigger({ type: "change", icon: icon.replace(op.iconClassFix, ''), code: Iconpicker.ICONSET[op.iconset].icons[icon.replace(op.iconClassFix, '')]}); //icon });
                el.find('input:hidden').val(Iconpicker.ICONSET[op.iconset].icons[icon.replace(op.iconClassFix, '')]); //ELI icon
                el.find('input:text').val(icon.replace(op.iconClassFix, ''));

//op.table.find('.search-control').val(icon.replace(op.iconClassFix, ''));
//                op.table.find('.search-control').trigger({ type: "change", icon: 'empty' });
            }
